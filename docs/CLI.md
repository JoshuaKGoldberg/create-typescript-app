...
| `--bundle`       | `boolean`  | Whether to bundle build output into a single file, per [FAQs > How can I bundle build output?](./FAQs.md#how-can-i-bundle-build-output)                            | Whether an existing `tsdown.config.ts` omits `unbundle: true`, or `false`                      |
| `--codecovToken` | `boolean`  | Whether to send a `CODECOV_TOKEN` repository secret to Codecov in CI, per [FAQs > How can I populate `CODECOV_TOKEN`?](./FAQs.md#how-can-i-populate-codecov_token) | Whether an existing `ci.yml` passes `CODECOV_TOKEN` to `codecov/codecov-action`, or `false`    |
