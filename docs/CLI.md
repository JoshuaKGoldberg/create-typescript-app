...
| `--codecovToken` | `boolean`  | Whether to send a `CODECOV_TOKEN` repository secret to Codecov in CI                                                                    | Whether an existing `ci.yml` passes `CODECOV_TOKEN` to `codecov/codecov-action`, or `false`    |
