...
| ------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `codecovSecret`     | Codecov upload token, encrypted for the repository, to store as its `CODECOV_TOKEN` secret | Fetched from the Codecov and GitHub APIs when `codecovToken` is on and `CODECOV_API_TOKEN` is set |
