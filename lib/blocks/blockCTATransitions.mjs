...
//#region src/blocks/blockCTATransitions.ts
const blockCTATransitions = base.createBlock({
	about: { name: "CTA Transitions" },
	produce({ options }) {
		return {
			addons: [blockPackageJson({ properties: { devDependencies: { "create-typescript-app": packageData.version } } }), blockRepositoryBranchRuleset({ requiredStatusChecks: ["Transition"] })],
			files: { ".github": {
				actions: { transition: { "action.yaml": formatYaml({
					...
					runs: {
						steps: [
							...
							},
							{
								...
								uses: resolveUses("stefanzweifel/git-auto-commit-action", "v5", options.workflowsVersions),
								with: {
							...
							{
								id: "package-change",
								uses: resolveUses("JoshuaKGoldberg/package-change-detector-action", "0.1.0", options.workflowsVersions),
								with: { properties: "engines" }
							},
							{
								if: `steps.package-change.outputs.changed == 'true'`,
								uses: resolveUses("JoshuaKGoldberg/draft-pull-request-once-action", "0.0.1", options.workflowsVersions),
								with: {
				...
				workflows: { "cta.yaml": createSoloWorkflowFile({
					jobName: "Transition",
					name: "CTA",
					...
					steps: [
						...
						},
						{
							...
							with: { token: "${{ secrets.ACCESS_TOKEN }}" }
						},
						{
							if: "steps.checkout.outcome == 'skipped'",
	...
	transition() {
		return { addons: [blockRemoveFiles({ files: [".github/actions/transition/action.yml", ".github/workflows/cta.yml"] })] };
	}
});
//#endregion
export { blockCTATransitions };
