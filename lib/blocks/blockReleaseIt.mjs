...
import { blockRepositorySecrets } from "./blockRepositorySecrets.mjs";
import { z } from "zod";
//#region src/blocks/blockReleaseIt.ts
const blockReleaseIt = base.createBlock({
	about: { name: "release-it" },
	addons: { builders: z.array(z.object({
		order: z.number(),
		run: z.string()
	})).default([]) },
		...
		return {
			addons: [
				blockPackageJson({ properties: {
					devDependencies: getPackageDependencies("@release-it/conventional-changelog", "release-it"),
					scripts: { "should-semantic-release": void 0 }
				} }),
				blockREADME({ badges: [{
					alt: "📦 npm version",
				...
				".github": { workflows: {
					"post-release.yaml": createSoloWorkflowFile({
						...
						steps: [
							...
							{
								uses: resolveUses("apexskier/github-release-commenter", "v1", options.workflowsVersions),
								with: {
									"comment-template": `
					...
					"release.yaml": createSoloWorkflowFile({
						...
						steps: [
							...
							...builders.sort((a, b) => a.order - b.order).map(({ run }) => ({ run })),
							{
								env: { GITHUB_TOKEN: "${{ secrets.ACCESS_TOKEN }}" },
								uses: resolveUses("JoshuaKGoldberg/release-it-action", "v0.4.0", options.workflowsVersions)
							}
						]
					})
				} },
			...
			suggestions: [[`- add ${options.owner}/${options.repository} and \`release.yaml\` as a Trusted Publisher on:`, `   https://www.npmjs.com/package/${options.repository}/access`].join("\n")]
		};
	},
	transition() {
		return { addons: [blockRemoveFiles({ files: [".github/workflows/post-release.yml", ".github/workflows/release.yml"] })] };
	}
});
//#endregion
export { blockReleaseIt };
