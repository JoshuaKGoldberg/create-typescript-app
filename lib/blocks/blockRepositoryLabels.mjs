...
import { outcomeLabels } from "./outcomeLabels.mjs";
import { determineLabelChanges } from "set-github-repository-labels";
//#region src/blocks/blockRepositoryLabels.ts
const blockRepositoryLabels = base.createBlock({
	about: { name: "Repository Labels" },
	produce({ options }) {
		const changes = determineLabelChanges(options.existingLabels ?? [], outcomeLabels);
		const requestData = {
			owner: options.owner,
			repo: options.repository
		};
		return { requests: changes.map((change) => {
			switch (change.type) {
				case "delete": return {
