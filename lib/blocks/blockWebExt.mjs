...
//#region src/blocks/blockWebExt.ts
const blockWebExt = base.createBlock({
	about: { name: "Web-ext" },
	produce() {
		return { addons: [
			blockCSpell({ ignorePaths: ["assets"] }),
...
`,
				...
				Linting: { contents: { items: [`- \`pnpm lint:web-ext\` ([web-ext](https://extensionworkshop.com)): Lints browser extension metadata after \`pnpm dev\` creates local files`] } }
			} }),
			...
			blockGitignore({ ignores: ["*.zip", "web-ext-artifacts"] }),
