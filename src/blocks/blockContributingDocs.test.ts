...
describe("blockContributingDocs", () => {
	test("without mode", () => {
		...
		expect(creation).toMatchInlineSnapshot(`
			...
			Thanks for your interest in contributing to \`test-repository\`! 💖
			...
			With the exception of very small typos, all changes to this repository generally need to correspond to an [unassigned open issue marked as \`status: accepting prs\` on the issue tracker](https://github.com/test-owner/test-repository/issues?q=is%3Aissue+is%3Aopen+label%3A%22status%3A+accepting+prs%22+no%3Aassignee+).
			If this is your first time contributing, consider searching for [unassigned issues that also have the \`good first issue\` label](https://github.com/test-owner/test-repository/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22+label%3A%22status%3A+accepting+prs%22+no%3Aassignee+).
			...
			Check [previously merged PRs](https://github.com/test-owner/test-repository/pulls?q=is%3Apr+is%3Amerged+-label%3Adependencies+) for reference.
