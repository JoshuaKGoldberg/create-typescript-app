...
import { ensureHydrationInputValues } from "./hydrationInputValues.js";

describe("ensureHydrationInputValues", () => {
	it("throws an error when no settings are filled out", () => {
		expect(() =>
			ensureHydrationInputValues({
				author: undefined,
...
- Could not determine title. Please add an h1 to the README.md, or provide with --title.`);
	...
	it("does not throw an error when all required settings are filled out", () => {
		expect(() =>
			ensureHydrationInputValues({
				author: "Test Author",
